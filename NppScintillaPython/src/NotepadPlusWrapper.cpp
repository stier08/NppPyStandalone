...
#include "NppScintillaPython/include/PythonScriptVersion.h"
...
BufferEncoding NotepadPlusWrapper::getBufferEncoding(intptr_t bufferID)
{
	...
	if (checkForValidBuffer(bufferID))
	{
		ret = static_cast<BufferEncoding>(callNotepad(NPPM_GETBUFFERENCODING, static_cast<WPARAM>(bufferID)));
	}
	else
	{
		invalidValueProvided("invalid aka unknown bufferID provided");
		//add some random return value to avoid compiler warning about missing return value
		ret = BufferEncoding::uniEnd;
	}
	return ret;
}
...
FormatType NotepadPlusWrapper::getBufferFormatType(intptr_t bufferID)
{
	...
	if (checkForValidBuffer(bufferID))
	{
		ret = static_cast<FormatType>(callNotepad(NPPM_GETBUFFERFORMAT, static_cast<WPARAM>(bufferID)));
	}
	else
	{
		invalidValueProvided("invalid aka unknown bufferID provided");
		//add some random return value to avoid compiler warning about missing return value
    	ret = FormatType::WIN_FORMAT;
...
{
	return callNotepad(NPPM_HIDETOOLBAR, 0, hideOrNot)!=0;
}

bool NotepadPlusWrapper::isToolBarHidden()
{
	return callNotepad(NPPM_ISTOOLBARHIDDEN) != 0;
}

bool NotepadPlusWrapper::hideMenu(bool hideOrNot)
{
	return callNotepad(NPPM_HIDEMENU, 0, hideOrNot)!= 0;
}

bool NotepadPlusWrapper::isMenuHidden()
{
	return callNotepad(NPPM_ISMENUHIDDEN) != 0;
}

bool NotepadPlusWrapper::hideStatusBar(bool hideOrNot)
{
	return callNotepad(NPPM_HIDESTATUSBAR, 0, hideOrNot) != 0;
}

bool NotepadPlusWrapper::isStatusBarHidden()
{
	return callNotepad(NPPM_ISSTATUSBARHIDDEN) != 0;
}
...
{
	return callNotepad(NPPM_ISDOCSWITCHERSHOWN) != 0;
}
...
{
	std::size_t size = static_cast<std::size_t>( callNotepad(NPPM_GETLANGUAGENAME, langType, NULL) );
...
{
	std::size_t size = static_cast<std::size_t> ( callNotepad(NPPM_GETLANGUAGEDESC, langType, NULL) );
	wchar_t* result(new wchar_t[size]);
...
{
	return callNotepad(NPPM_GETAPPDATAPLUGINSALLOWED, 0, 0) !=0;
}
...
{
	return callNotepad(NPPM_MAKECURRENTBUFFERDIRTY) != 0;
}

bool NotepadPlusWrapper::getEnableThemeTextureFunc()
{
	return callNotepad(NPPM_GETENABLETHEMETEXTUREFUNC) != 0;
}
